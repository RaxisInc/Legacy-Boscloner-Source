...
void ModThenAcquireRawAdcSamples125k(int delay_off, int period_0, int period_1,
		uint8_t *command) {
	int at134khz;
...
//	int n = GraphTraceLen;
	...
	uint32_t sampleslo = (FSAMPLE << 4) / FREQLO, sampleshi = (FSAMPLE << 4)
			/ FREQHI;
	...
	for (i = 0; i < n - 1; i++) {
		...
		if ((dest[i] < 0) && (dest[i + 1] > 0)) {
			...
			if (cycles > 15) {
				...
				if ((samples > (sampleslo - threshold))
						&& (samples < (sampleslo + threshold))) {
					// low frequency represents a 1
					shift3 |= (1 << 31);
				} else if ((samples > (sampleshi - threshold))
	...
	if (cycles != 0xF0B) {
		...
		Dbprintf("Info: Tag data: %x%08x, crc=%x", (unsigned int) shift1,
				(unsigned int) shift0, (unsigned int) shift2 & 0xFFFF);
...
void WriteTItag(uint32_t idhi, uint32_t idlo, uint16_t crc) {
	...
	Dbprintf("Writing to tag: %x%08x, crc=%x", (unsigned int) idhi,
			(unsigned int) idlo, crc);
...
void CmdHIDdemodFSK(int findone, int *high, int *low, int ledcontrol) {
	...
	for (;;) {
		...
		for (idx = 0; idx < m - 6; idx++) {
			...
			if (dest[idx] && dest[idx + 1] && dest[idx + 2] && (!dest[idx + 3])
					&& (!dest[idx + 4]) && (!dest[idx + 5])) {
				found = 1;
				idx += 6;
				if (found && (hi2 | hi | lo)) {
					if (hi2 != 0) {
						Dbprintf("TAG ID: %x%08x%08x (%d)", (unsigned int) hi2,
								(unsigned int) hi, (unsigned int) lo,
								(unsigned int) (lo >> 1) & 0xFFFF);
					} else {
						Dbprintf("TAG ID: %x%08x (%d)", (unsigned int) hi,
								(unsigned int) lo,
								(unsigned int) (lo >> 1) & 0xFFFF);
					}
...
void T55xxWriteBlock(uint32_t Data, uint32_t Block, uint32_t Pwd,
		uint8_t PwdMode) {
	unsigned int i;
...
Dbprintf("CopyHIDtoT55x7 function:");
Dbprintf("hi2x:%x hix:%x lox:%x longfmt:%x");	//trying to understand how to correct data is sent
Dbprintf("hi2:%u hi:%u lo:%u longfmt:%u");
	...
	T55x7_MODULATION_FSK2a | last_block << T55x7_MAXBLOCK_SHIFT, 0, 0, 0);
...
void WriteEM410x(uint32_t card, uint32_t id_hi, uint32_t id_lo) {
	...
	if (card) {
		...
		T55x7_MODULATION_MANCHESTER | 2 << T55x7_MAXBLOCK_SHIFT, 0, 0, 0);
	} else
		// Writing configuration for T5555(Q5) tag
		T55xxWriteBlock(0x1F << T5555_BITRATE_SHIFT |
		T5555_MODULATION_MANCHESTER | 2 << T5555_MAXBLOCK_SHIFT, 0, 0, 0);
...
void CopyIndala64toT55x7(int hi, int lo) {
	...
	T55x7_MODULATION_PSK1 | 2 << T55x7_MAXBLOCK_SHIFT, 0, 0, 0);
...
void CopyIndala224toT55x7(int uid1, int uid2, int uid3, int uid4, int uid5,
		int uid6, int uid7) {
	...
	T55xxWriteBlock(T55x7_BITRATE_RF_32 |
	T55x7_MODULATION_PSK1 | 7 << T55x7_MAXBLOCK_SHIFT, 0, 0, 0);
...
int DemodPCF7931(uint8_t **outBlocks) {
	...
	for (bitidx = 0; i < GraphTraceLen; i++) {
		if ((GraphBuffer[i - 1] > GraphBuffer[i] && dir == 1
				&& GraphBuffer[i] > lmax)
				|| (GraphBuffer[i - 1] < GraphBuffer[i] && dir == 0
						&& GraphBuffer[i] < lmin)) {
			lc = i - lastval;
			lastval = i;
				...
				if (bitidx == 128) {
					for (j = 0; j < 16; j++) {
						Blocks[num_blocks][j] = 128 * BitStream[j * 8 + 7]
								...
								+ 8 * BitStream[j * 8 + 3]
								+ 4 * BitStream[j * 8 + 2]
								+ 2 * BitStream[j * 8 + 1] + BitStream[j * 8];
					}
					num_blocks++;
				}
				bitidx = 0;
...
int IsBlock0PCF7931(uint8_t *Block) {
	// Assume RFU means 0 :)
	if ((memcmp(Block, "\x00\x00\x00\x00\x00\x00\x00\x01", 8) == 0)
			&& memcmp(Block + 9, "\x00\x00\x00\x00\x00\x00\x00", 7) == 0) // PAC enabled
		return 1;
	if ((memcmp(Block + 9, "\x00\x00\x00\x00\x00\x00\x00", 7) == 0)
			&& Block[7] == 0) // PAC disabled, can it *really* happen ?
		return 1;
	return 0;
}
	...
	do {
		...
		for (i = 0; i < n; i++)
			Dbprintf(
					...
					tmpBlocks[i][0], tmpBlocks[i][1], tmpBlocks[i][2],
					tmpBlocks[i][3], tmpBlocks[i][4], tmpBlocks[i][5],
					tmpBlocks[i][6], tmpBlocks[i][7], tmpBlocks[i][8],
					tmpBlocks[i][9], tmpBlocks[i][10], tmpBlocks[i][11],
					tmpBlocks[i][12], tmpBlocks[i][13], tmpBlocks[i][14],
					tmpBlocks[i][15]);
		if (!ident) {
			for (i = 0; i < n; i++) {
				if (IsBlock0PCF7931(tmpBlocks[i])) {
					...
					if (i < n - 1 && IsBlock1PCF7931(tmpBlocks[i + 1])) {
						...
						Blocks[1][ALLOC] = 1;
						max_blocks = max((Blocks[1][14] & 0x7f), Blocks[1][15])
								+ 1;
						// Debug print
						Dbprintf("(dbg) Max blocks: %d", max_blocks);
						num_blocks = 2;
						// Handle following blocks
						for (j = i + 2, ind2 = 2; j != i;
								j++, ind2++, num_blocks++) {
							if (j == n)
								j = 0;
							if (j == i)
			...
			for (i = 0; i < n; i++) { // Look for identical block in known blocks
				if (memcmp(tmpBlocks[i],
						"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
						16)) { // Block is not full of 00
					for (j = 0; j < max_blocks; j++) {
						if (Blocks[j][ALLOC] == 1
								&& !memcmp(tmpBlocks[i], Blocks[j], 16)) {
							// Found an identical block
							for (ind = i - 1, ind2 = j - 1; ind >= 0;
									ind--, ind2--) {
								if (ind2 < 0)
									ind2 = max_blocks;
	...
	for (i = 0; i < max_blocks; i++) {
		if (Blocks[i][ALLOC] == 1)
			Dbprintf(
					...
					Blocks[i][8], Blocks[i][9], Blocks[i][10], Blocks[i][11],
					Blocks[i][12], Blocks[i][13], Blocks[i][14], Blocks[i][15]);
		else
...
void EM4xWriteWord(uint32_t Data, uint8_t Address, uint32_t Pwd,
		uint8_t PwdMode) {
